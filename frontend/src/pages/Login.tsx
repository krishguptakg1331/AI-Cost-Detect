...
export default function Login({ setIsAuthenticated }: { setIsAuthenticated: (val: boolean) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail?.message || 'Failed to login');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center">
      <div className="glass-panel w-full max-w-md p-8 rounded-2xl relative overflow-hidden">
        ...
        <div className="absolute -top-24 -right-24 w-48 h-48 bg-indigo-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob"></div>
        ...
        <div className="relative z-10">
          <div className="flex justify-center mb-8">
            <div className="bg-white/5 p-4 rounded-full border border-white/10 shadow-lg">
              <LogIn className="w-8 h-8 text-indigo-400 drop-shadow-[0_0_8px_rgba(99,102,241,0.8)]" />
            </div>
          </div>
          
          <h2 className="text-3xl font-bold text-center text-white mb-2 bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">Welcome Back</h2>
          <p className="text-center text-gray-400 mb-8">Sign in to continue to AI Cost Detective</p>
